...
class Solution {
    ...
    // Memoization
    private static int rec(int i, int j, int[][] mat, boolean[][] vis, int[][] dp) {
        // Base case
        if(i < 0 || j < 0 || i >= mat.length || j >= mat[0].length || vis[i][j] == true || mat[i][j] == 0 || isValid(i, j, mat) == false) {
            return max;
        }
        ...
        // if we reach last column, then return 1
        if(j == mat[0].length - 1) {
            return 1;
        }
