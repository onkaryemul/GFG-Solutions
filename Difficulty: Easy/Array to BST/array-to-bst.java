...
class Solution {
    ...
    public Node sortedArrayToBST(int[] nums) {
        // Code here
        Node root = getHeightBalancedBST(nums, 0, nums.length-1);
        return root;
    }
    
    
    private Node getHeightBalancedBST(int[] nums, int left, int right) {
        // Base case
        if(left > right) {
            return null;
        }
        ...
        root.left = getHeightBalancedBST(nums, left, mid-1);
        root.right = getHeightBalancedBST(nums, mid+1, right);
