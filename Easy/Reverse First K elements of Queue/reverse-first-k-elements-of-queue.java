...
class GfG {
    ...
    public Queue<Integer> modifyQueue(Queue<Integer> q, int k) {
        ...
        // Push first k integers from given queue into stack
        for(int i=0; i<k; i++) {
            st.push(q.poll());
        }
        
        // Pop each integer from stack and add it to ans queue
        while(!st.isEmpty()) {
            ans.add(st.pop());
        }
        
        // finally, remove remaining elements from given queue and add it to the ans queue
        while(!q.isEmpty()) {
            ans.add(q.poll());
        }
